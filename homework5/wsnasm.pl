...
sub getRegNr {
  my $lreg = 0 + substr($_[0],1,length($_[0]));
  if ( (substr($_[0],0,1) ne "r")  || ($lreg < 1) || ($lreg > 10) ) {
    $lreg = 0; 
  }
  return($lreg);
}

### parse and return int8 value
sub getInt8 {
  my $lval = 0 + $_[0];
...
sub codeGen {
  ...
  while (<$file>) {
    ...
    if ($arg1 eq "none") {
      ...
      push(@image,$code . sprintf("%01X",$reg1));
    }  
#    elsif ($arg1 eq "uint8") {
#      $val1 = getUInt8($marg1);
...
#    }
     ...
     assert(0,"unexpected first argument");
   }
   ...
   # check second argument type and emit code
    ...
    if ($arg2 eq "none") {
      ...
      $val2 = getInt8($marg2);
      assert(($val2 != 255),"second argument must be an int [-127..127]");
      my $valstr = sprintf("%02X",$val2);
      $valstr = substr($valstr,length($valstr)-2,2);
      push(@image,$valstr);
    }
    elsif ($arg2 eq "label") {
      $label = $marg2;
      ...
      assert(0,"unexpected second argument");
    }
...
# produce code for message handler

@msg = codeGen(\*INPUT,"--Msg--");
print "msg code segment: @msg\n";
...
@metadata = (4+@init+@timer+@msg,0+@init,0+@timer,0+@msg);
$metabin = pack("c4",@metadata);
...
# build msg handler binary

$msgstr = "";
for (my $i; $i < @msg; $i++) {
  $msgstr = $msgstr . $msg[$i];
}
$msgbin = pack("H*",$msgstr);
...
print 0+@metadata+@init+@timer+@msg . " bytes\n";
