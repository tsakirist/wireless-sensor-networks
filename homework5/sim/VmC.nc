...
implementation {
    ...
    bool appTimerIsRunning(uint8_t);
    bool isMyParent(uint16_t);
    ...
    void startCollisionTimer() {
        ...
        if(flood || siblings == 0) {
            delay = call Random.rand16() % CLSN_DELAY;
        }
        if(siblings != 0) {
    ...
    void startTimer(uint8_t indx, uint8_t mode, uint16_t interval) {
        ...
        if(mode == 0) {
            ...
            dbg("Timer", "Timer in aggregation mode, interval: %d\n", interval);
        }
        switch(indx) {
            case 0:
    ...
    void sendValue() {
        ...
        if(temp.originator == TOS_NODE_ID) {
            ...
            dbg("Buffer", "***********\n");
            dbg("Buffer", "Sending results back to the serial...\n");
            dbg("Buffer", "reg7: %hhu, reg8: %hhu, originator: %hhu, app_id: %hhu\n", temp.reg7, temp.reg8, temp.originator, temp.id);
            if(temp.path_indx > 0) {
                if(temp.reg7 > 1 && temp.reg8 > 1) {
                    dbg("Buffer", "Nodes: ");
                }
                else {
                    dbg("Buffer", "Path: ");
                }
                for(i=0; i<temp.path_indx; i++) {
                    if(i != temp.path_indx -1) {
                        dbg_clear("Buffer", "%hhu -> ", temp.path[i]);
                    }
                    else {
                        dbg_clear("Buffer", "%hhu\n", temp.path[i]);
                    }
                }
            }
            dbg("Buffer", "***********\n");
        ...
        if(global_period == 2 && children) {
            dbg("Buffer", "pkt_send: %hhu\n", pkt_send);
            pkt_send++;
            dbg("Buffer", "pkt_send: %hhu\n", pkt_send);
            if(pkt_send == all_nodes + 1) {
                dbg("Buffer", "pkt_send == all_nodes + 1, posting informMsg()\n");
                post informMsg();
            }
        }
    }
    ...
    void setAppUniPayload(uint8_t indx) {
        ...
        temp.id         = curr_app->id;
        temp.path_indx  = 0;
        dbg("Buffer", "Inside unicast payload! seq: %hhu, reg7: %hhu, reg8: %hhu, originator: %hhu, id: %hhu\n",
            temp.seq, temp.reg7, temp.reg8, temp.originator, temp.id);
    ...
    void terminateApp(uint8_t id, uint8_t originator) {
        ...
        for(i=0; i<num_apps; i++) {
            if(apps[i].id == id && apps[i].originator == originator && apps[i].active) {
                dbg("MySerial", "Terminating app with id: %hhu, originator: %hhu\n", id, originator);
                apps[i].active = FALSE;
                active_apps--;
                stopTimer(i);
                closeLed(i);
        ...
        dbg("MySerial", "No need to terminate app with id: %hhu, originator: %hhu\n", id, originator);
    }
    ...
    void save(app_msg_t *app) {
        ...
        if((new_app->bin_len <= MAX_PAYLOAD)) {
            ...
            active_apps++;
            post interpreter();
            dbg("MySerial", "originator: %hhu, id: %hhu, bin_len: %hhu, init_len: %hhu, timer_len: %hhu\n", new_app->originator, new_app->id, new_app->bin_len, new_app->init_len, new_app->timer_len);
            dbg("MySerial", "Application: ");
            for(i=0; i<(new_app->indx); i++) {
    ...
    void extend(app_msg_t *app) {
        ...
        if((new_app->init_len + new_app->timer_len + new_app->msg_len) == new_app->indx) {
            ...
            dbg("MySerial", "Application: ");
            for(i=0; i<(new_app->init_len + new_app->timer_len + new_app->msg_len); i++) {
                dbg_clear("MySerial", "%x\t", new_app->buf[i]);
            }
            dbg_clear("MySerial", "pos: %hhu\n", pos);
    ...
    void executeInstruction(application_t *curr_app, uint8_t code, uint8_t arg1, int16_t arg2) {
        ...
        switch(code) {
            ...
            /* In case we are in the timer_handler */
                ...
                dbg("Buffer", "1.Inside ret, path:");
                for(i=0; i<p_track[app_indx].path_indx; i++) {
                    dbg_clear("Buffer", "%hhu ", p_track[app_indx].path[i]);
                }
                dbg("Buffer", "\n");
            }
    ...
    task void bufferResult() {
        ...
        dbg("Buffer", "Inside bufferReuslt, global_period: %hhu, path_indx: %hhu\n", global_period, umsg->path_indx);
        /* If it is 1-hop neighbor e.g child, take into consideration only the first period */
        if(global_period == 1) {
            if(umsg->path_indx == 1) {
                saveChild(umsg->path[0]);
                dbg("Buffer", "Received packet with path_indx = 1, children: %hu\n", children);
            }
            all_nodes++;
            dbg("Buffer", "all_nodes: %hhu\n", all_nodes);
        }
        if(pos == -1) {
            ...
            dbg("Buffer", "umsg->seq: %hhu, apps[pos].seq: %hhu\n", umsg->seq, apps[pos].seq);
            ...
            if(apps[pos].aggr_mode) {
                ...
                if(((apps[pos].pc > apps[pos].init_len) && (apps[pos].pc <= apps[pos].init_len + apps[pos].timer_len)) || (apps[pos].seq != umsg->seq)) {
                    ...
                    if(period_counter != 2) {
                        ...
                        aggr_back_off++;
                        dbg("Buffer", "Packet received late, aggr_back_off: %hhu\n", aggr_back_off);
                        call Leds.led0Toggle();
                    }
                    mw_send = TRUE;
                    dbg("Buffer", "mw_send: TRUE\n");
                }
            }
            if(mw_send) {
            ...
            if(apps[pos].pc == apps[pos].init_len && !apps[pos].timer_fired) {
                ...
                dbg("Buffer", "No event handler, path:");
                for(i=0; i<p_track[pos].path_indx; i++) {
                    dbg_clear("Buffer", "%hhu ", p_track[pos].path[i]);
                }
                dbg("Buffer", "\n");
                post interpreter();
            }
            ...
            if(apps[app_indx].active) {
                if(!apps[app_indx].waiting) {
                    ...
                    if(apps[app_indx].pc < apps[app_indx].init_len) {
                        dbg("Interpreter", "Inside init_handler\n");
                        break;
                    }
                    /* Timer_handler */
                        ...
                        dbg("Interpreter", "Inside timer_handler timer_fired = TRUE\n");
                        break;
                    }
                    /* Message_handler */
                    else if((apps[app_indx].pc >= apps[app_indx].init_len + apps[app_indx].timer_len) && apps[app_indx].received) {
                        dbg("Interpreter2", "Inside message_handler received = TRUE\n");
                        break;
                    }
                }
                checked++;
    ...
    task void broadcastMsg() {
        if(!busy) {
            ...
            if(call BroadcastAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(broadcast_msg_t)) == SUCCESS) {
                updateSeq();
                dbg("Transmit", "Node %hu Broadcasting application..\n", TOS_NODE_ID);
                busy = TRUE;
            }
        }
        else {
    ...
    task void forwardMsg() {
        if(!busy) {
            ...
            if(call BroadcastAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(broadcast_msg_t)) == SUCCESS) {
                dbg("Transmit", "Node %hu Forwarding application..\n", TOS_NODE_ID);
                updateFwdIndx();
                busy = TRUE;
            }
        }
    ...
    task void unicastMsg() {
        if(!busy) {
            ...
            *temp = call SendQueue.dequeue();
            ...
            dbg("Transmit", "Inside unicast, addr: %hhu\n", addr);
        ...
        if(!busy) {
            ...
            if(temp->addr == AM_BROADCAST_ADDR) {
                if(call BroadcastAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(term_msg_t)) == SUCCESS) {
                    dbg("Transmit", "Flooding terminate message\n");
                    busy = TRUE;
                }
            }
            else {
                if(call UnicastAMSend.send(temp->addr, &pkt, sizeof(term_msg_t)) == SUCCESS) {
                    dbg("Transmit", "Unicasting terminate message\n");
                    busy = TRUE;
                }
            }
        }
    ...
    task void informMsg() {
        if(!busy) {
            ...
            imsg->siblings = children > 0 ? children -1 : children;
            if(call BroadcastAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(inform_msg_t)) == SUCCESS) {
                dbg("Transmit", "Sending siblings update message! @ %s\n", sim_time_string());
                busy = TRUE;
            }
        }
    }
    ...
    event message_t *BroadcastReceive.receive(message_t *msg, void *payload, uint8_t len) {
        if(len == sizeof(inform_msg_t)) {
            ...
            if(isMyParent(prev_hop)) {
                ...
                dbg("Receive", "Received siblings update, siblings: %hhu\n", siblings);
            }
        }
        else if(len == sizeof(term_msg_t)) {
            dbg("Receive", "Received a broadcast termination message\n");
    ...
    event message_t *UnicastReceive.receive(message_t *msg, void *payload, uint8_t len) {
        if(len == sizeof(term_msg_t)) {
            dbg("Receive", "Received unicast terminate from: %hhu , at @ %s\n",(call RadioAMPacket.source(msg)), sim_time_string());
            tmsg = *(term_msg_t *)payload;
            ...
            uint8_t i;
            dbg("Receive", "Received unicast result from: %hhu , at @ %s\n",(call RadioAMPacket.source(msg)), sim_time_string());
            umsg = (unicast_msg_t *)payload;
            prev_hop = call RadioAMPacket.source(msg);
            post bufferResult();
            dbg("Receive", "Received reg[7]: %hhu, reg[8]: %hhu, orig: %hhu, id: %hhu\n",
                umsg->reg7, umsg->reg8, umsg->originator, umsg->id);
            dbg("Receive", "Path: ");
            for(i=0; i<umsg->path_indx; i++) {
                if(i!=umsg->path_indx-1) {
                    dbg_clear("Receive", "%hhu -> ", umsg->path[i]);
                }
                else {
                    dbg_clear("Receive", "%hhu\n", umsg->path[i]);
                }
            }
        }
        return msg;
    ...
    event message_t *SerialReceive.receive(message_t *msg, void *payload, uint8_t len) {
        if(len == sizeof(app_msg_t)) {
            ...
            dbg("MySerial", "\n");
            dbg("MySerial", "Received application message! @ %s\n", sim_time_string());
            post broadcastMsg();
            saveApplication(amsg);
            dbg("MySerial", "\n");
        }
        else if(len == sizeof(serial_term_msg_t)) {
            serial_term_msg_t *stmsg = (serial_term_msg_t *)payload;
            setTermPayload(stmsg->id, TOS_NODE_ID, AM_BROADCAST_ADDR);
            dbg("MySerial", "Received terminate message for application id: %hhu, originator: %hhu @ %s\n", tmsg.id, tmsg.originator, sim_time_string());
            terminateApp(tmsg.id, tmsg.originator);
        }
        return msg;
    }
